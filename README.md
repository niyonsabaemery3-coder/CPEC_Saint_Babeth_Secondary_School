...
**1. Start the backend first** (full instructions in `server/README.md`):
...
**2. Then start the frontend**, in a second terminal, from the project root:
...
deploys it automatically every time you push to `main`.
...
3. In the repo, go to **Settings → Secrets and variables → Actions → Variables** and add a repository
   variable `VITE_API_URL` set to your deployed backend's URL (see `server/README.md` for hosting options).
   The workflow passes this through at build time.
4. Push (or re-push) to `main`. Check the **Actions** tab — once the "Deploy to GitHub Pages" workflow
