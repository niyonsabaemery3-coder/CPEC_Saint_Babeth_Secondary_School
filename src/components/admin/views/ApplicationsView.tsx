...
export default function ApplicationsView() {
  ...
  const [templates, setTemplates] = useState(applicationTemplates);
  const [templatesSaved, setTemplatesSaved] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);

  useEffect(() => { fetchApplications().catch(() => {}); }, []);
  useEffect(() => { fetchApplicationTemplates().catch(() => {}); }, []);
  useEffect(() => setTemplates(applicationTemplates), [applicationTemplates]);
  ...
  const openReview = (application: StudentApplication) => {
    setSelected(application); setReviewStatus(application.status); setFeedback(application.feedback || applicationTemplates[application.status]); setAttachment(null);
  };
  ...
  return <div className="admin-panel-view active">
    <div className="application-templates-panel">
      ...
      {templatesOpen && <div className="application-templates-body">
        ...
        {(["pending", "approved", "rejected"] as const).map((key) => <div className="ffield always-float" key={key}><textarea value={templates[key]} onChange={(event) => setTemplates((current) => ({ ...current, [key]: event.target.value }))} placeholder=" " /><label>{key[0].toUpperCase() + key.slice(1)} message</label></div>)}
    ...
    <form className="application-filters" onSubmit={filter}>
      ...
      <select value={status} onChange={(e) => setStatus(e.target.value as typeof status)}><option value="all">All statuses</option><option value="pending">Pending</option><option value="approved">Approved</option><option value="rejected">Rejected</option></select>
      <button className="a-add-btn" type="submit"><i className="fa-solid fa-filter" /> Filter</button>
    </form>

    <div className="a-table-wrap"><table className="a-table"><thead><tr><th>Applicant</th><th>Track / Date</th><th>Parent / Phones</th><th>Status</th><th>Report</th><th>Actions</th></tr></thead><tbody>
      {visible.length === 0 ? <tr><td colSpan={6} className="a-empty">No applications found for this filter.</td></tr> : visible.map((application) => <tr key={application.id}>
        ...
        <td><span className={`application-status application-status-${application.status}`}>{application.status}</span></td>
    ...
    {selected && <div className="application-review-overlay" role="dialog" aria-modal="true"><div className="application-review-modal">
      ...
      <label className="review-label">Decision</label><div className="review-status-buttons">{(["pending", "approved", "rejected"] as const).map((option) => <button type="button" key={option} className={reviewStatus === option ? `active ${option}` : ""} onClick={() => { setReviewStatus(option); setFeedback(applicationTemplates[option]); }}>{option}</button>)}</div>
